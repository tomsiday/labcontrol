...
delay_pos_init = 0 # A bodge, for now,
...
def time_scan():
	global delay_position
	if playing_time:
		if delay_position >= (delay_pos_init-delay_length):
			new_data = dict()
			
			print("PW" + str(delay_position)) # Print the position being sent to the KLINGER
			klinger.write("PW" + str(delay_position)) # Send the position to the KLINGER
			#I NEED TO GET IT TO WAIT UNTIL KLINGER IS READY.
			time.sleep(t_wait) # Wait for the MFLI filter to settle
			
			out = daq.getSample('/%s/demods/%d/sample' % (device, demod_index))
			out['R'] = np.abs(out['x'] + 1j*out['y']) # Calculate the magnitude R from x and y
			print(out) # 
			new_data['x'] = t_plot_line.data_source.data['x'] + [delay_position] #
			new_data['y'] = t_plot_line.data_source.data['y'] + [out['R'][0]]
			t_plot_line.data_source.data = new_data
		
			curdoc().add_next_tick_callback(time_scan)
			delay_position = delay_position - delay_step # Set the next time step (in time, as it is converted above)
...
# Set position of delay stage
def call_starttime(attrname, old, new):
	global delay_position
	global delay_pos_init
	
	delay_position = int(new)
	delay_pos_init = int(new)
	
	print('Old position' + old)
	print('New position' + new)
	klinger.write('PW' + str(new))
	
def call_stepsize(attrname, old, new):
	global delay_step
	delay_step = int(new)
	
def call_delay_length(attrname, old, new):
	global delay_length
	delay_length = int(new)
	print("Scan length now" + str(delay_length))
	
def call_wait(attrname, old, new):
	global t_wait
	t_wait = int(new)
	print("Dwell time set to: " + str(t_wait))
...
# playing is a variable which is used to stop 'neverending' loops	TODO: The stop time takes lioger the longer the graph as been running? maybe something 'datay'			
playing = True
...
t_but_stop = Button(label='Stop') # Stop NOW button
t_but_stop.on_click(call_t_but_stop) # Call the stop time scan callback function
t_in_starttime = TextInput(value = str(delay_position), title="Scan start time:") # Input the start time of the scan (in ps)
t_in_starttime.on_change('value', call_starttime)
t_in_length = TextInput(value = str(delay_length), title="Scan length:") # Input the length of the scan (in ps)
t_in_length.on_change('value', call_delay_length)
t_in_stepsize = TextInput(value = str(delay_step), title="Scan step size:") # Input size of step (picoseconds). (If length/stepsize is not an integer, round)
t_in_stepsize.on_change('value', call_stepsize)
t_in_wait = TextInput(value = str(t_wait), title="Dwell time (s):")
t_in_wait.on_change('value', call_wait)
