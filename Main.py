...
TODO:
    * BUG: Bottom "Move delay stage to" uses TW value not it's own
    * Try open(file, ... buffering=1) for line buffering
...
class ESP301:
    ...
    def position(self, axis, pos=None, wait=False):
        ...
        if pos is not None: # If position is given, then set position
...
class Main(QtGui.QMainWindow, Ui_MainWindow): # PyQt4 GUI window class.
    ...
    def XTScan(self): # Loop to run the XT scan (including collect and plot data)
        ...
        print("► XT Scan start : %s ◄               " % starttime.strftime("%H:%M:%S"), end='', flush=True)
        # generate filename string
        basename = ("%4d%02d%02d-%02d%02d" % (starttime.year, starttime.month, starttime.day, starttime.hour, starttime.minute))
        metafilename = (basename + "-XTScan-metadata.txt")
        datafilename = (basename + "-XTScan.txt")
        ...
        metafile.write("\t\"XTSpaceInit\":%s,\n" % int(XT_pos_space_init))
        metafile.write("\t\"XTSpaceStep\":%s,\n" % int(XTStepSpace))
        metafile.write("\t\"XTSpaceLength\":%s,\n" % int(XTLengthSpace))
        metafile.write("\t\"DelayInit\":%s,\n" % int(XT_pos_time_init))
        metafile.write("\t\"DelayLength\":%s,\n" % int(XTLengthTime))
        metafile.write("\t\"DelayStep\":%s,\n" % int(XTStepTime))
        metafile.write("\t\"Date\":\"%s\",\n" % datetime.date.today())
        metafile.write("\t\"StartTime\":\"%s\",\n" % starttime.strftime("%H:%M:%S"))
        metafile.write("\t\"DwellTime\":%f,\n" % t_wait)
        metafile.write("\t\"TimePoints\":%d,\n" % int(len(t)))
        metafile.write("\t\"SpacePoints\":%d,\n" % int(len(xy)))
        if self.ui.ScanAlongY.isChecked() is True: # Scan changes y values (channel 2)
            metafile.write("\t\"ScanAlong\":\"Y\"\n")
        if self.ui.ScanAlongX.isChecked() is True: # Scan changes x values (channel 2)
            metafile.write("\t\"ScanAlong\":\"X\"\n")
        metafile.write("}\n")
        metafile.close()
        for a in range(0, len(t)): # loop across number of time samples
            ...
            if self.ui.ScanAlongY.isChecked() is True: # Scan changes y values (channel 2)
                ...
                XYscanner.position(2, xy[0]*1e-3)
            if self.ui.ScanAlongX.isChecked() is True: # Scan changes x values (channel 2)
                # set x position (1e-3 to convert microns to mm)
                XYscanner.position(1, xy[0]*1e-3)
            time.sleep(t_wait)
    ...
    def TimeScan(self):
        ...
        # generate filename string
        basename = ("%4d%02d%02d-%02d%02d" % (starttime.year, starttime.month, starttime.day, starttime.hour, starttime.minute))
        metafilename = (basename + "-TWScan-metadata.txt")
        datafilename = (basename + "-TWScan.txt")
        metafile = open(metafilename, 'w')
